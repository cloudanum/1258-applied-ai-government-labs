...
## Goal
Draft one complete cleaning prompt you could hand to an LLM to fix the dirty 311 rows from activity 6.A, with explicit normalization rules and a required output shape. Your artifact is the prompt itself, posted in chat, with a rules section (ISO dates, dedup by request ID) and an output contract (JSON with keys `rows`, `flags`, `removed_duplicate_ids`).
...
**🧩 Pattern watch:** Design pattern, **Deterministic rules, checkable output**: explicit rules plus a JSON contract you can test. Anti-pattern, **Vague cleanup request**: 'please fix the data' returns data you cannot audit.
...
5. Post your complete cleaning prompt in Zoom chat (or as a Mural comment in person), keeping the rules section and the output-shape section clearly separated.
6. Read one other learner's prompt and reply with the first thing that would go wrong if you ran it: an ambiguous rule, a missing key, or no instruction for rows that cannot be fixed.
...
Writing instructions with no output contract: "clean this data" returns confident prose, so require a rules section and an explicit output schema the result can be validated against.
