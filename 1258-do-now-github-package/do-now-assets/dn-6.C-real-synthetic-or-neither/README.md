...
## Goal
Sort six data-need stickies (D1–D6) into **Real data**, **Synthetic data**, or **Neither**, and flag the one placement with the biggest privacy implication. Your artifact is the six placements plus one privacy concern explained in a sentence.
...
2. Read all six data-need stickies D1–D6: a pre-approval rules prototype (D1), production fraud prediction (D2), a classroom exercise on missing values (D3), estimating income from confidential tax records (D4), a chatbot load test (D5), and an annual equity audit of a live system (D6).
...
4. Drag each sticky into **Real**, **Synthetic**, or **Neither**. Remote? Post placements in Zoom chat, e.g. `D1-Synthetic, D2-Real, D4-Neither...`.
5. Flag the placement with the biggest privacy implication (look hard at D4 and D6) and comment with the concern in one sentence.
6. Compare boards with a neighbor, disagreements between D2 and D4 are the teachable moment, then initial or color-tag your stickies before the instructor reveals the key.
...
Assuming synthetic data is always the privacy-safe default: it can still leak patterns from its source data, and it can never substitute for real outcomes in an audit.
