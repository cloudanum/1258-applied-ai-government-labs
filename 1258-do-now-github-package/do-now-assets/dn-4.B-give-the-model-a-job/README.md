...
## Goal
Pick one government task sticky (J1–J4) and write a two-part prompt for it: a role, "You are a ___", plus an output contract, "the output must look like ___." You walk away with one complete role-plus-contract prompt posted in chat.
...
## Run steps 🪜
1. Read the task stickies J1–J4 and pick the one closest to your real work.
2. Ask who in government would do this task best; that is your role, written as `You are a ___` (for a records-request task: `You are a government records officer`).
3. Decide what a finished answer must contain: for a records summary, issues, exemptions to check, and a draft response in 5 bullets; for a chatbot review, factual risk, tone risk, and a revised answer.
4. Post your full prompt to Zoom chat as one structured message: `Task: ___ | Role: You are a ___ | Output must look like: ___`.
5. Find a classmate whose different role would produce a different answer and reply saying what would change.
6. If the instructor is capturing on Mural, copy your role-plus-contract onto a sticky beside your chosen task.
