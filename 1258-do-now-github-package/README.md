...
- `1258-DoNow-Master-Guide-Standalone_custom.html` / `_custom.pdf`: same guide plus a "Targeted students" block on every activity, naming up to 3 of the 11 enrolled students whose intake-sheet interests match the activity, with a short note on why it helps them. Built by `customize_guide.py`.
- `1258-DoNow-Master-Guide-Standalone_custom_filter.html` / `_custom_filter.pdf`: student filter edition. Sticky filter bar shows only the activities relevant to one student (about 40 percent each), plus a static Student relevance map. Built by `filter_guide.py`.
